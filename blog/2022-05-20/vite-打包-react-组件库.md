...
### 2) 为什么说 Vite 生产环境仍需打包

很多同学可能会认为是浏览器 ESM 兼容性，或者是网络请求太多的问题，实际上都不是，下面从两个角度分析：

**1. 前端业务工程**

对于前端业务工程来说，不打包意味着源码直接部署，这种情况下我们需要做一些处理，这样才能让浏览器能看得懂我们的代码

- JSX、TS、Vue、Sass、Less 等语法转换
- 某些第三方库用 CommonJS 模块规范，需要转为 ES Module
- ES2015+ 语法、API 兼容处理
- 代码混淆压缩，以减小体积

这样我们就必须在生产环境也搞一套类似开发环境的机制，例如一个请求打过来，需要在服务端解析一下文件路径、读取文件内容、调用 ESBuild 做一下 transform 再返回，这样无疑增加了服务端压力，而且对用户来说，资源响应速度肯定变慢了。反过来如果业务工程经过编译、打包，那就可以直接作为静态资源部署到 nginx 或者 CDN 上面，这样还能进行客户端缓存，提升资源加载速度。所以宁可在构建阶段多花一些时间，如果仅仅为了不打包而增加服务端压力、牺牲资源加载性能，就有点得不偿失。

:::tip

补充一下：

不打包不意味着不能对资源进行 `transform`。father 4 支持 Bundless 和 Bundle 两种构建模式：
...
![image](./bundled-and-bundleless.png)

在不打包的模式下，对”所有源码文件单独编译、平行输出“，个人理解应该是不进行依赖分析，只是扫描工程目录下所有文件，单独进行 `transform`。这样的话，肯定就没办法处理 `node_modules` 下的依赖了（例如需要 Babel 语法转换、兼容），另外 npm 包发包有时也需要将 `node_modules` 进行打包。所以 father 4 增加了一个依赖预打包的功能：

- 一是 NPM 包发布后安装体积更小、速度更快
- 二是不担心三方依赖更新引起 Bug
- 三是NPM 包发布后安装 0 warning

依赖预打包好处很多，业务工程安装依赖速度更快，业务工程 Webpack 打包也无需进行额外的依赖分析，同时还能避免第三方库间接依赖不受控问题（默认锁定大版本，允许小版本和补丁版本更新）。但是需要注意，预打包很容易造成模块冗余，例如一个 React 组件库，在构建的时候如果把 React 打包进去，不仅导致业务工程模块冗余，还破坏单例模式，导致运行时 bug。

:::

**2. 第三方库**

对于第三方库来说，确实可以源码直接发包，由业务工程统一处理。但是这样就会有一个问题，需要业务工程的 babel-loader 处理 `node_modules` 下的模块，这样显然非常耗时。如果第三方库经过编译、打包，这样业务工程的 babel-loader 就可以缩小作用范围，提升构建效率。
